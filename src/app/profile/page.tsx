...
function ProfileContent({ user }: any) {
    ...
    const profileMutation = useMutation({
        mutationFn: (data: any) => axiosInstance.put("/user/profile", {
            name: data.name,
            avatar: data.avatar,
            preferences: { budget: Number(data.budget), interests: data.interests.split(",").map((s: string) => s.trim()).filter(Boolean), travelStyle: data.travelStyle },
        }),
        onSuccess: () => { setProfileMsg("Profile updated!"); queryClient.invalidateQueries({ queryKey: ["profile"] }); },
    ...
    const passwordMutation = useMutation({
        ...
        onSuccess: () => { setPasswordMsg("Password changed!"); resetPassword(); },
        ...
        <div className="max-w-2xl mx-auto px-4 py-12 mt-16">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-2xl shadow-lg mb-8">
                ...
                <form onSubmit={handleProfileSubmit((data) => profileMutation.mutate(data))} className="space-y-4">
                    <input {...regProfile("name")} placeholder="Name" className="w-full px-4 py-2 border rounded-xl" />
                    <input {...regProfile("avatar")} placeholder="Avatar URL" className="w-full px-4 py-2 border rounded-xl" />
                    <input {...regProfile("budget")} type="number" placeholder="Budget ($)" className="w-full px-4 py-2 border rounded-xl" />
                    <input {...regProfile("interests")} placeholder="Interests (comma-separated)" className="w-full px-4 py-2 border rounded-xl" />
                    <input {...regProfile("travelStyle")} placeholder="Travel style" className="w-full px-4 py-2 border rounded-xl" />
                    <button type="submit" className="w-full py-2 bg-emerald-500 text-white rounded-xl font-semibold hover:bg-emerald-600">{profileMutation.isPending ? "Saving..." : "Update Profile"}</button>
                </form>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white p-6 rounded-2xl shadow-lg">
                ...
                <form onSubmit={handlePasswordSubmit((data) => passwordMutation.mutate(data))} className="space-y-4">
                    ...
                    <button type="submit" className="w-full py-2 bg-amber-400 text-white rounded-xl font-semibold hover:bg-amber-500">{passwordMutation.isPending ? "Changing..." : "Change Password"}</button>
                </form>
            </motion.div>
        </div>
    );
