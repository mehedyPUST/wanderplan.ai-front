            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                {isLoading ? (
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-16">
                        ...
                        <Link href="/explore" className="inline-flex items-center gap-2 bg-emerald-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-emerald-600 transition-colors">Explore Destinations</Link>
                    </motion.div>
                ) : (
                        ...
                        {wishlist.map((item: any, i: number) => (
                            <motion.div key={item._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow border border-gray-100">
                                <div className="flex justify-between items-start mb-3">
                                    <div>
                                        <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2"><FaMapMarkerAlt className="text-rose-500" />{item.name}</h3>
                                        {item.country && <p className="text-sm text-gray-500 mt-1">{item.country}</p>}
                                    </div>
                                    <motion.button whileTap={{ scale: 0.9 }} onClick={() => deleteMutation.mutate(item.slug)} className="text-gray-400 hover:text-red-500 transition-colors p-2"><FaTrash /></motion.button>
                                </div>
