...
export default function LoginPage() {
    ...
    const handleDemoLogin = () => {
        setValue("email", "console@test.com");
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    <div>
                        ...
                        <input id="email" type="email" {...register("email")} className="mt-1 w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-emerald-400 focus:outline-none" placeholder="you@example.com" />
                        {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email.message}</p>}
                    </div>
                    <div>
                        <label htmlFor="password" className="block text-sm font-medium text-gray-700">Password</label>
                        <input id="password" type="password" {...register("password")} className="mt-1 w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-emerald-400 focus:outline-none" placeholder="••••••••" />
